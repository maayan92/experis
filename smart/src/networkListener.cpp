...
void NetworkListener::ListenEvents()
{
    ...
    for (;;) {
        ...
        if (numOfEvents == -1) {
            LOGINFO(m_logFile, "epoll_wait failed!");
        }
        ...
        for (int i = 0; i < numOfEvents; ++i) {
            if (events[i].data.fd == m_server.GetSockNum()) {
                ...
                if (clientSock == -1) {
                    LOGINFO(m_logFile, "Client connection failed, with socket number: " + clientSock);
                    continue;
                }
...
bool NetworkListener::createEpoll()
{
    ...
    if(-1 == m_epollFd) {
        ...
        if(ENFILE == errno) {
            LOGINFO(m_logFile, "Reached the limit of the total number of open files!");
            return false;
        }
        if(ENOMEM == errno) {
            LOGINFO(m_logFile, "Insufficient memory to create the kernel object!");
            return false;
        }
        assert(!"undocumented error for epoll_create1");
    }
    LOGINFO(m_logFile, "epoll created successfully!");
    return true;
}
...
bool NetworkListener::setNonBlocking(int a_socket)
{
	...
	if (-1 == flags) {
        LOGINFO(m_logFile, "failed to get file access mode and file status flags, with socket number: " + a_socket);
        return false;
	}

    int status = ::fcntl(a_socket, F_SETFL, flags | O_NONBLOCK);
	if(-1 == status) {
		LOGINFO(m_logFile, "failed to set file descriptor as non blocking, with socket number: " + a_socket);
        return false;
	}
...
bool NetworkListener::addEntry(int a_socket)
{
    ...
    if (-1 == status) {
        if(EEXIST == errno) {
            LOGINFO(m_logFile, "The file descriptor is already registered with this epoll instance, with socket number: " + a_socket);
            return false;
        }
        if(ENOMEM == errno) {
            LOGINFO(m_logFile, "There was insufficient memory to add the file descriptor, with socket number: " + a_socket);
            return false;
        }
        if(ENOSPC == errno) {
            LOGINFO(m_logFile, "Imposed limit was encountered while trying to register a new file descriptor, with socket number: " + a_socket);
            return false;
        }
        assert(ENOENT == errno);
        HandleEpollCtlError();
    }
    LOGINFO(m_logFile, "client connected successfully!");
    return true;
}
...
void NetworkListener::removeEntry(int a_socket)
{
    ...
    if (-1 == status) {
        if(ENOENT == errno) {
            LOGINFO(m_logFile, "The file descriptor is not registered with this epoll instance, with socket number: " + a_socket);
            return;
        }
        assert(EEXIST == errno);
        assert(ENOMEM == errno);
    ...
    LOGINFO(m_logFile, "client closed successfully!");
}
...
void NetworkListener::receiveMsg(int a_socket)
{
    ...
    if(0 == read_bytes) {
        ...
        if(EINTR == errno) {
            LOGINFO(m_logFile, "The receive was interrupted by a signal before any data was available, with socket number: " + a_socket);
        }
    }
